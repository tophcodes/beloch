(** The enumeration pipeline: candidate combos of alignments -> symbolic
    ...
    restricted to combos without AL10 gives 203 of the paper's 489 [§4], and
    with AL10 gives the full 489 -- both symbol-exact against the paper's
    printed listing (566 candidates -> 494 strict survivors -> 489 published;
    see notes/2026-08-04-multifold-phase1-reproduction.md). *)
...
(** {!run_twofold}'s result, additionally restricted by
    ...
    [alperin2006]; at [with_al10:true], symbol-exact against the paper's full
    489 (566 candidates -> 494 strict survivors -> 489 published). Logs both the
    unfiltered ({!run_twofold}) and R4-filtered counts to stderr, since R4 is
    not a derived rule and its effect should stay visible to callers rather than
    silently included. *)
