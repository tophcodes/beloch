...
let strict_keep : classify_result -> bool = function
  | `Zero_dim { count; multiplicity_free } -> count >= 1 && multiplicity_free
  ...
  let rec go pos = function
    ...
    | tok :: rest ->
        let eqs, denoms, pos' =
          ...
          | "A2" ->
              (* FLF(L1) <-> L2: reflected L1 equals given L2 — 2 eqs. *)
              ...
              in
              (eq_pair img (line_triple l2), [ d ], pos2)
          | "A3" ->
...
(* Two-fold: candidates -> Symeq -> Msolve, 4 variables (two folds).
   ...
   lax_only share one ~1099-combo msolve sweep instead of each re-solving
   it. *)
