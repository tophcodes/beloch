...
let kind_name (k : alignment_kind) : string =
  match k with
  | AlOnto (AoPoint _, AoPoint _) -> "point onto point"
  | AlOnto (AoPoint _, AoLine _) -> "point onto line"
  | AlOnto (AoLine _, AoPoint _) -> "line onto point"
  | AlOnto (AoLine _, AoLine _) -> "line onto line"
  | AlThrough _ -> "through a point"
  | AlPerp _ -> "perp to a line"

let unrecognised (c : construction) : 'a =
  Error.fail c.c_span
    ("these alignments are not one of the seven axioms: "
    ^ String.concat ", " (List.map (fun a -> kind_name a.al_kind) c.c_alignments))
...
let names_fold_lines (c : construction) : bool =
  c.c_fold_lines <> []
  || List.exists
       (fun a -> a.al_fold_line <> None || a.al_fold_line2 <> None)
       c.c_alignments

let axiom_of_construction (c : construction) : axiom =
  if names_fold_lines c then
    Error.fail c.c_span "a construction over named fold lines is not evaluated yet";
  ...
  List.iter
    (fun a ->
      ...
      | AlOnto (AoPoint p, AoPoint q) -> onto_pp := (p, q) :: !onto_pp
      | AlOnto (AoPoint p, AoLine l) -> onto_pl := (p, l) :: !onto_pl
      | AlOnto (AoLine l, AoLine m) -> onto_ll := (l, m) :: !onto_ll
      | AlOnto (AoLine _, AoPoint _) -> other := true
      | AlThrough p -> through := p :: !through
      | AlPerp l -> perp := l :: !perp)
    c.c_alignments;
  ...
  let one_line () =
    ...
    if c.c_toward <> None then
      Error.fail c.c_span "this construction determines one line; drop toward"
  in
  if !other then unrecognised c
    ...
    | [], [ (p, d) ], [], [], [ m ] ->
        one_line ();
        MapOntoLine (p, d, m)
    | [], [], [ (l, m) ], [], [] -> MapLines (l, m, c.c_toward)
    | [], [ (p, d) ], [], [ q ], [] -> MapThrough (p, d, q, c.c_toward)
    | [], [ (p, d); (q, e) ], [], [], [] -> MapBoth (p, d, q, e, c.c_toward)
    | _ -> unrecognised c

let bound_name (out : output) : string option =
  match out with
  | Anonymous -> None
  | Named (n, _, _) | Into (n, _) -> Some n
