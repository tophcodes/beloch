...
(* ---- Public extraction: text lines paired with their parsed form ---- *)

let extract (src : string) : (string * assertion) list =
  String.split_on_char '\n' src
  |> List.filter_map is_assert_line
  |> List.map (fun line ->
         match parse_assertion (tokenize line) with
         | a -> (line, a)
         | exception Harness_fail msg -> harness_fail "%S: %s" line msg)

let expected_error (assertions : assertion list) : string option =
  match List.filter (function AExpectError _ -> true | _ -> false) assertions with
  | [] -> None
  | [ AExpectError substr ] ->
      if List.length assertions <> 1 then
        harness_fail "'expect error' must be the only assertion in the file"
      else Some substr
  | _ :: _ :: _ -> harness_fail "more than one 'expect error' assertion"
  | [ _ ] -> assert false (* the filter only ever keeps AExpectError elements *)
...
let check (fd : Eval.folded) (a : assertion) : unit =
  match a with
...
let check_error_message ~(expected : string) (msg : string) : unit =
  if not (contains_substring msg expected) then
    harness_fail "error message %S does not contain %S" msg expected
