...
- **Suffixes.** The paper's a/b suffix names which fold does the folding
  [alperin2006, l. 484-489]. At k=3 suffixes range over $\{a, b, c\} \triangleq$
  $\{F1, F2, F3\}$, and the suffix symmetry group is **S3** (all six relabelings
  of the three folds), not S2. I write $F_i(\cdot)$ for reflection across fold i,
  and use index notation $AL5_{i \to j}$ etc. in the body; a concrete symbol
...
- **Word length.** Any alignment can be rewritten (Def. 12 style) as
  $u(A) \leftrightarrow v(B)$ with $u, v$ words in $\{F_1, F_2, F_3\}$ and $A, B$ given
  objects, fold lines, or virtual points. Its *total word length* is $|u| + |v|$
  after reduction. Two facts used throughout:
  - **Involution** $F_i(F_i(\cdot)) = \mathrm{id}$ [alperin2006, l. 208-217].
  - **Incidence transport**: $P \in \ell \iff F_i(P) \in F_i(\ell)$ (reflections
    preserve incidence — this is what makes $F(P) \leftrightarrow L \equiv F(L) \leftrightarrow P$
    [alperin2006, l. 279-283]).
  - Def-12 moves (applying some $F_i$ to both sides [alperin2006,
    l. 476-478]) change each side's length by $\pm 1$, so the **parity of the
...
- **Single-alignment-determines-a-fold** shapes ($F(P1) \leftrightarrow P2 \triangleq O2$,
...
- **$F_aF_b(P) \leftrightarrow LFb$** (i.e. $\rho(P)$ lies on fold b's line). Transport with
  $F_b$ gives $F_bF_aF_b(P) \in LFb$ (longer); with $F_a$ gives $F_b(P) \in F_a(LFb)$ — a
...
- **$F_a(LFb) \leftrightarrow F_b(LFa)$**. Writing the folds' angle as $\theta$, `AL1` is
  ...
  $\rho^n = \mathrm{id}$ family. Finite-paper-verifiable (align two visible crease
  images), one equation, nondegenerate. Absent.
- Depth ≥ 3 on givens: $\rho^2(P) \leftrightarrow P'$, or reflection across the derived
  mirror $F_a(LFb)$ (word $F_aF_bF_a$) — by the parity/length invariant these can
  never reduce to total length ≤ 2, so they are structurally outside. The
  family $\{\rho^n = \mathrm{id}\}$, $\{\rho^n(P) \leftrightarrow P'\}$ is **infinite**: an alphabet
  closed under Def-12 with *no* depth cutoff does not exist as a finite object.
...
the exclusion is **load-bearing for finiteness** (the $\rho^n$ slope). The paper
...
alignments become irreducible: $F_1F_2(P) \leftrightarrow F_3(P')$ has minimal total
length 3, and no Def-12 move brings both sides to depth ≤ 1 (parity + reduced-word
...
model, where a region behind two creases moves by $\rho_{ij}$. The choice is no
...
|---|---|---|---|---|
| $AL1_{ij}$ | $F_i(LFj) \leftrightarrow LFj \iff \rho_{ij}^2 = \mathrm{id}$ | 1 | 3 (unordered $\{i,j\}$: involution makes $F_i(LFj)\leftrightarrow LFj$ and $F_j(LFi)\leftrightarrow LFi$ the same constraint, $\theta=\pi/2$) | orbit of 3; each variant fixed by the transposition (ij) |
| $AL2_i$ | $F_i(L) \leftrightarrow L$ | 1 | 3 | orbit of 3; fixed by the (jk) transposition |
| $AL3_i$ | $LFi \leftrightarrow P$ | 1 | 3 | same |
| $AL4_{i \to j}$ | $F_i(L) \leftrightarrow LFj$ | 2 | 6 (ordered, $i\neq j$) | free orbit of 6 (no symmetry) |
| $AL5_{i \to j}$ | $F_i(P) \leftrightarrow LFj$ | 1 | 6 | free orbit |
| $AL6_i$ | $F_i(P) \leftrightarrow L$ | 1 | 3 | fixed by (jk) |
| $AL7_{i \to j}$ | $F_i(P) \leftrightarrow F_j(L)$ | 1 | 6 | free orbit |
| $AL8_{ij}$ | $F_i(P1) \leftrightarrow F_j(P2)$ | 2 | 3 (unordered: swapping folds = relabeling P1,P2, free under Def. 11 [alperin2006, l. 474-475]) | fixed by (ij) |
| $AL9_{ij}$ | $F_i(L1) \leftrightarrow F_j(L2)$ | 2 | 3 (unordered, same argument) | fixed by (ij) |
| $AL10_{i \to j}$ | $F_j(P_{LFi,L1}) \leftrightarrow L2$ | 1 | 6 (i = fold making the intersection, $j \neq i$ reflects; $j = i$ is trivial — the point lies on its own mirror) | free orbit |
| **$N1_{i;jk}$** | **$F_i(LFj) \leftrightarrow LFk$**, $i,j,k$ distinct | 2 | 3 (i chosen; $\{j,k\}$ unordered by involution) | fixed by (jk) |
...
N1 adds $k \cdot \binom{k-1}{2}$ more (0 at k=2 — which is why Fig. 4 couldn't contain
it — 3 at k=3).
...
l. 481-483]; $F_i(L) \leftrightarrow F_j(L)$ with the same L not counted (≡ $AL2_i$ + $AL2_j$)
...
(§1.2's $\rho^n$ slope). Two concrete cutoffs, chosen so that each has a
...
have i, j, k distinct; 6 variants each ($\rho_{ij}$ ordered, k determined):

| kind | shape | eqs | reading |
|---|---|---|---|
| N4 | $\rho_{ij}(P) \leftrightarrow LFk$ | 1 | doubly-folded point lands on the third crease |
| N5 | $\rho_{ij}(L) \leftrightarrow LFk$ | 2 | doubly-folded line lands on the third crease |
| N8 | $\rho_{ij}(P) \leftrightarrow F_k(P')$ | 2 | doubly-folded point meets the third flap's point image |
| N9 | $\rho_{ij}(L) \leftrightarrow F_k(L')$ | 2 | line version |
| N10 | $\rho_{ij}(P) \leftrightarrow F_k(L)$ | 1 | mixed |
...
line-line + 24 point-line = 48 symbols; this includes $\rho^2(P) \leftrightarrow P'$ in the
disguise $\rho_{12}(P) \leftrightarrow \rho_{21}(P')$). Two warnings, both load-bearing:

1. **The closure leaks.** Example: $F_2F_3(P) \leftrightarrow F_1(LF2)$ is per-side
...
2. **No non-arbitrary stopping point.** B2 excludes depth-3 words
   ...
   ($F_iF_jF_k(P) \leftrightarrow P' \iff F_jF_k(P) \leftrightarrow F_i(P')$ = N8) but not against fold
...
line: $F_{LFb}(P_{LFa,L1}) \leftrightarrow L2$. It comes in a and b varieties [l. 493-494].
...
vacuous → fair game), $P_{Fi,L} \leftrightarrow LFj$ could and didn't (precedent of
...
- **4 + 2** — two folds pinned first by a genuine 2FA (4 eqs among
  alignments whose suffixes $\subseteq \{i,j\}$), remainder a 1FA for the third fold
...
of alignments whose suffixes $\subseteq S$; the combo is separable iff $E(S) = 2|S|$
...
Six unknowns $x_1,y_1,\ldots,x_3,y_3$ (plus Rabinowitsch variable); saturate all
three isotropic factors $x_i^2 + y_i^2$ alongside the chart denominators —
...
single-reflection core + N1 + the 7 fold$\cap$fold virtual-point symbols) as the
...
4. **Excluded-with-precedent kinds** (N2/N3/N6, $P_{Fi,L} \leftrightarrow LFj$): confirm
...
5. **The R4-analogue stance — flagged as the no-oracle risk.** At k=2, R4
   ...
   constraints through directions and the $\rho_{ij}$, which are themselves
   dependent via $\rho_{13} = \rho_{12}\rho_{23}$) will produce borderline cases with
