...
let collapse (st : Fold_state.t) (es : elem list) ~(over : (int * int) list) :
  ...
  | Some o ->
      if n < 4 || n mod 2 = 1 then Error e_count
        ...
        if not (closure_ok o rays) then Error e_kawasaki
        else begin
          ...
          if abs (nm - nv) <> 2 then Error e_maekawa
            ...
            let constraints =
              List.init n (fun j ->
                  ...
                  let _, e = rays.(j) in
                  let eff =
                    e.valley <> (Isometry.det_sign tsec.(l) < 0)
                  in
            ...
            if valid = [] then Error e_selfint
              ...
              if filtered = [] then Error e_contra
                ...
                | [ (_, rank) ] ->
                    (* normalize: the bottom sector keeps the identity. *)
                    let b = ref 0 in
                    for s = 0 to n - 1 do
                      if rank.(s) < rank.(!b) then b := s
                    done;
